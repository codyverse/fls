...
class FLS:
    """
    A class for managing file match patterns using custom `.fls` protocol files.

    This class scans directories starting from a root path, reads `.fls` files to load match rules, and
    ...
    Attributes:
        ...
        _protocol (str): The filename of the protocol file to be processed (default: `.fls`).
    ...
    class FLSRule:
        ...
        def __init__(self, pattern, context='', parent=None):
            if parent is None or not isinstance(parent, FLS):
                raise Exception("Cannot instantiate FLSRule directly.")
    ...
    def __init__(self, root, protocol='.fls'):
        """
        Initialize the FLS instance.


        This method initializes the FLS class by setting the root directory from where the scanning starts and
        ...
        Args:
            ...
            protocol (str): The name of the protocol file to use for pattern matching.
                            Default is `.fls`. The file must be readable, and should contain valid match patterns.
        ...
        Example:
            # Create an instance of FLS with the current directory as the root
            fls = FLS(root='.', protocol='.ignorem')
        """
            ...
            with open(filepath, 'r') as file:
                for line in file:
                    ...
                    if pattern and not pattern.startswith('#'):
                        rule = self.FLSRule(pattern, parent=self, context=context)
    ...
    def get_all_rules(self):
        ...
        Returns:
            dict: A dictionary where the keys are directory paths and the values are lists of 
                  `FLSRule` objects associated with those directories.
        """
    ...
    def get_rules(self, directory):
        ...
        Returns:
            list: A list of `FLSRule` objects associated with the specified directory.
        """
    ...
    def add_rule(self, directory, pattern):
        ...
        pattern = pattern.strip()
        if pattern and not pattern.startswith('#'):
            rule = self.FLSRule(pattern, parent=self)
            self._rules[directory].append(rule)


if __name__ == '__main__':
    fls = FLS(root='.', protocol='.fls')
